...
public class ProductReviewServiceTest {
    ...
    public void merchantCanReplyOnlyToOwnProductReview() {
        ...
        when(reviews.findById(9L)).thenReturn(review); when(products.queryProductById(3L)).thenReturn(product); when(reviews.save(review)).thenReturn(review);
    ...
    public void publicReviewPageExposesOnlyRepositoryPageAndMetadata() {
        ...
        when(reviews.findByProductIdAndStatusOrderByIdDesc(org.mockito.Matchers.eq(3L), org.mockito.Matchers.eq("PUBLISHED"), any(Pageable.class)))
    ...
    public void buyerCanReportPublishedReviewOnceWithReason() {
        ...
        when(reviews.findById(9L)).thenReturn(review);
    ...
    @Test(expected = com.own.face.trade.TradeException.class)
    public void prohibitedTermsRejectReviewBeforeItIsSaved() {
        ...
        new ProductReviewService(reviews, mock(ProductReviewReportRepository.class), products, mock(OrderReviewEligibilityClient.class), new ReviewContentPolicy("blocked phrase")).create(1L, 3L, command);
    }
}
