...
async function load() { loading.value = true; try { const result = await catalogApi.products(filters) as any; rows.value = result.items || []; total.value = Number(result.total || 0); } finally { loading.value = false; } }
...
<template><main class="page"><Card :bordered="false"><Tag color="cyan">买家中心</Tag><h1>商品浏览</h1><p>仅展示当前可售商品，价格与库存以提交订单时的服务端校验为准。</p></Card><Card :bordered="false"><Form layout="inline" @finish="search"><Form.Item label="关键词"><Input v-model:value="filters.keyword" allow-clear /></Form.Item><Form.Item label="分类"><Select v-model:value="filters.categoryId" allow-clear :field-names="{label:'name',value:'id'}" :options="categories" style="width:160px" /></Form.Item><Space><Button html-type="submit" type="primary">搜索</Button><Button @click="Object.assign(filters,{categoryId:undefined,keyword:''});search()">重置</Button></Space></Form></Card><Card :bordered="false"><Table :columns="columns" :data-source="rows" :loading="loading" :pagination="{ current: filters.page + 1, pageSize: filters.size, total, showSizeChanger: false }" row-key="id" @change="changePage"><template #bodyCell="{ column, record }"><Space v-if="column.key === 'actions'"><Button type="link" @click="openDetail(record)">详情</Button><Button type="link" @click="add(record)">加入购物车</Button></Space></template></Table></Card><Drawer :open="!!detail" title="商品详情" width="560" @close="detail=undefined"><pre v-if="detail">{{ JSON.stringify(detail, null, 2) }}</pre></Drawer></main></template>
