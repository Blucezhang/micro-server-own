...
var app=angular.module("filterModuleAudit", ['ngResource']);
//定义拦截器工厂
app.factory('userInterceptorAudit',function($q) {
    var userInterceptorAudit = {
     		   ...
     		   if(responseErr.status=='500' && responseErr.data!=null && responseErr.data.error=='530' )
	   			{
	   			    /*$.messager.alert("用户未登录或会话过期，请重新登录！");*/
	   			    alert("用户未登录或会话过期，请重新登录！");
	   				window.location.href="/WebSite/Audit/index.jsp";
	   			}
...
/*    			if(responseErr.status=='0')
    ...
    return userInterceptorAudit;
});
//将自定义拦截器放入拦截器栈
app.config(function($httpProvider) {
    $httpProvider.interceptors.push('userInterceptorAudit');
});
