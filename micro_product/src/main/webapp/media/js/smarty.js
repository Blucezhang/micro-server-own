...
app.controller("SmartyController", ["$scope", "$document", "smartySuggestor", "$window", "$timeout",
    ...
    return {
        ...
        template: '<p ng-repeat="suggestion in suggestions" ' +
            ...
            'ng-click="selectSuggestion()"> '+
                '{{suggestion}} ' +
            '</p>' +
            '<p style="margin-bottom:0px;" ng-mouseover="applyClass({x:suggestions.length})" ' +
            /*'ng-class="{selected: suggestions.length == selected}" ' +*/
            'ng-click="selectSuggestion()" class="show-all well well-sm"> ' +
            '索引内容： "{{prefix}}" &raquo;</p>'
    };
}]);
